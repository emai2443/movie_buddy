    ...
    fetch(url)
      ...
      .then((data) => {
        if (data.results.length !== 0) {
          //If there are movies with selected genre, show them
          setMovies(data.results);
          let v = document.getElementById("emptyResult");
          v.innerHTML = ``;
        } else {
          //If no movies with selected genre, show message "No Results"
          setMovies(data.results);
          let v = document.getElementById("emptyResult");
      ...
      <div>
        ...
        <div className="filterACButton">
          <div className="filterButton">
            <button onClick={() => filter()} className="applyButton">
              Apply
            </button>
          </div>
          <div className="filterButton">
            <button
              onClick={() => (clearBtn(), filter())}
